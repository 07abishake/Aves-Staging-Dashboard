...
function Departments() {
    ...
    const handleEditSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`https://api.avessecurity.com/api/Department/update/${editDepartmentData.id}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
    ...
    const handleEditClick = (department) => {
        setEditDepartmentData({
            ...
            parentId: department.parentDepartment ? department.parentDepartment._id : '',
            leadId: department.leadName ? department.leadName._id : '',
            assignUsers: department.assignUsers ? department.assignUsers.map(user => ({
                value: user._id,
                label: user.username
            })) : [],
        });
                            ...
                            <form onSubmit={handleEditSubmit} style={{ height: "500px" }}>
                                <div className="d-flex mb-3 justify-content-between">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) =>
                                            setEditDepartmentData({ ...editDepartmentData, name: e.target.value })
                                        }
                                        style={{ width: "67%" }}
                                        required
                                    />
                                ...
                                <div className="mb-3 d-flex justify-content-around">
                                    ...
                                    <select
                                        ...
                                        onChange={(e) =>
                                            setEditDepartmentData({ ...editDepartmentData, parentId: e.target.value })
                                        }
                                    >
                                        <option value="">Select Department</option>
                                        {departments
                                ...
                                <div className="mb-3 d-flex justify-content-around">
                                    ...
                                    <Select
                                        options={leads}
                                        value={leads.find(lead => lead.value === editDepartmentData.leadId) || null}
                                        onChange={(selected) =>
                                            setEditDepartmentData({ 
                                                ...editDepartmentData, 
                                ...
                                <div className="text-end">
                                    <button type="button" className="btn" onClick={() => setShowEditCanvas(false)}>
                                        Cancel
                                    </button>
                                    <button type="submit" className="btn btn-primary" disabled={loading}>
                                        {loading ? "Saving..." : "Save"}
