...
const ProductManagement = () => {
  ...
  const fetchParentProductRequests = async () => {
    try {
      console.log('🔄 Fetching parent product requests...');
      const response = await productAPI.getParentProductRequests();
      
      console.log('📋 Parent requests response:', response.data);
      ...
      setError('Failed to load product requests: ' + (error.response?.data?.message || error.message));
    }
  };
  ...
  const fetchProductNameSuggestions = async (query) => {
    if (!query || query.length < 2) {
      setSuggestions(prev => ({ ...prev, productNames: [] }));
      setShowDropdown(prev => ({ ...prev, productNames: false }));
      return;
    }

    try {
      const response = await productAPI.getProductNameSuggestions(query);
      const newSuggestions = response.data.data || [];
      setSuggestions(prev => ({ ...prev, productNames: newSuggestions }));
      setShowDropdown(prev => ({ ...prev, productNames: newSuggestions.length > 0 }));
    } catch (error) {
  ...
  const checkDuplicateProduct = async (productData) => {
    try {
      setIsCheckingDuplicate(true);
      const response = await productAPI.checkDuplicate(productData);
  ...
  const handleInputChange = async (field, value) => {
    if (field.includes('.')) {
      ...
      // Handle product name input specifically
      if (field === 'name') {
        ...
        // Fetch suggestions for product names
        setTimeout(() => {
        ...
        if (value.length > 3) {
          ...
          if (duplicateCheck.isDuplicate) {
            setDuplicateProduct(duplicateCheck.existingProduct);
          } else {
            setDuplicateProduct(null);
          }
        }
  ...
  // Enhanced product name selection with auto-fill
  const handleProductNameSelect = (product) => {
    console.log('🔄 Auto-filling form with product:', product);
    
    // Auto-fill all form fields with selected product data
    const updatedFormData = {
      ...
      currentQuantity: product.currentQuantity || product.initialQuantity || 0,
    ...
    // Update image preview if product has images
    ...
    // Clear suggestions and set duplicate product
    setSuggestions(prev => ({ ...prev, productNames: [] }));
    setShowDropdown(prev => ({ ...prev, productNames: false }));
    setDuplicateProduct(product);
    
    setSuccess(`Form auto-filled with "${product.name}" data!`);
  };
  ...
  const handleCreateProduct = async (e) => {
    ...
    if (duplicateCheck.isDuplicate && duplicateCheck.existingProduct) {
      setError(`Product "${formData.name}" already exists with similar details. Please check the existing product or modify the details.`);
      setLoading(false);
      return;
    }
  ...
  // UPDATED: Enhanced handleAssignment function with module creation
  const handleAssignment = async (assignmentType, moduleData = null, existingModuleId = null) => {
    ...
    try {
      ...
      if (assignmentType === 'Regular' && moduleData) {
        // Create new module and assign
        console.log('Creating new module with data:', moduleData);
        
        // First create the module
        const moduleResponse = await moduleAPI.create(moduleData);
        console.log('Module created:', moduleResponse.data);
        
        if (moduleResponse.data.success) {
          const newModule = moduleResponse.data.data;
          
          // Then assign product to this module
          response = await productAPI.assign(createdProduct._id, {
            assignmentType: 'Regular',
            existingModuleId: newModule._id
          });
        }
      } else if (assignmentType === 'Regular' && existingModuleId) {
        ...
        // Other assignment types (First Aid Kit, Authorized Inventory)
        response = await productAPI.assign(createdProduct._id, {
  ...
  const handleRequestProduct = async () => {
    ...
    try {
      const requestPayload = {
        ...
        sourceOrg: selectedProduct.sourceOrganization || selectedProduct.sourceOrg,
  ...
  const handleProductRequestAction = async (requestId, action, notes = '', approvedQuantity = null) => {
    try {
      ...
      if (action === 'APPROVE' && approvedQuantity) {
        requestData.approvedQuantity = parseInt(approvedQuantity);
      }
...
// UPDATED: Enhanced Assignment Modal Component with Logo Upload
...
//       // For now, we'll store the file and preview
//       setModuleData(prev => ({
//         ...prev,
//         logo: file,
...
//         logo: moduleData.logo, // This would be handled by file upload in backend
...
//       // This will create the module AND the dynamic table
//       await onAssign('Regular', submitData, null);
...
//               <Form.Label>Module Logo (Optional)</Form.Label>
//               <div className="d-flex align-items-center gap-3">
//                 {moduleData.logoPreview ? (
//                   <div className="position-relative">
//                     <img 
//                       src={moduleData.logoPreview} 
...
//                       cursor: 'pointer'
//                     }}
//                     onClick={() => document.getElementById('moduleLogoUpload').click()}
//                   >
...
//                     Upload a logo for this module (optional)
//                   </Form.Text>
//                 </div>
//               </div>
//               {uploadingLogo && (
...
//                 placeholder="Enter module name (e.g., Emdoor Checklist, Safety Equipment)"
//                 required
//               />
//               <Form.Text className="text-muted">
//                 This will create a dynamic table named: module_{moduleData.name.replace(/[^a-zA-Z0-9]/g, '_')}
//               </Form.Text>
//             </Form.Group>
...
//                     <Col md={5}>
//                       <Form.Control
//                         placeholder="Field Name (e.g., Torch, Bandage, Location)"
...
//                   <small>Example fields: Name, Location, Quantity, Status, etc.</small>
//                 </div>
//               )}
//             </div>
...
//             {/* Information about what happens when module is created */}
//             <Alert variant="info" className="mt-3">
//               <h6>What happens when you create a module?</h6>
//               <ul className="mb-0">
//                 <li>A new module record is created with your defined fields</li>
//                 <li>A dynamic database table is automatically created</li>
//                 <li>You can then fill data into this module's form</li>
//                 <li>The product will be linked to this module</li>
//               </ul>
//             </Alert>
//           </div>
//         )}
...
//                           <ul className="mb-0">
//                             {selectedModule.fields?.map((field, idx) => (
//                               <li key={idx}>
//                                 {field.fieldName} ({field.fieldType}) 
//                                 {field.isMandatory && <Badge bg="warning" className="ms-1">Required</Badge>}
//                               </li>
//                             ))}
//                           </ul>
//                         </div>
...
// UPDATED: Assignment Modal with Predefined Schema Fields
const EnhancedAssignmentModal = ({ show, onHide, onAssign, product, existingModules }) => {
  const [assignmentType, setAssignmentType] = useState('');
  const [showModuleOptions, setShowModuleOptions] = useState(false);
  const [showModuleCreation, setShowModuleCreation] = useState(false);
  ...
  //   setCreatingModule(true);
  //   try {
  //     // Prepare module data - ONLY name and logo
  //     const submitData = {
  //       name: moduleData.name,
  //       logo: moduleData.logo
  //     };

  //     console.log('Creating module with predefined schema fields:', submitData);
      
  //     // This will create the module with your predefined EmtelephoneChecklistSchema
  //     await onAssign('Regular', submitData, null);
  //     setShowModuleCreation(false);
  //     resetModuleData();
  //   } catch (error) {
  ...
  setCreatingModule(true);
  try {
    // Use ONLY the allowed field types: Text, Number, Checkbox, Image
    const defaultFields = [
      { fieldName: "Status", fieldType: "Text", isMandatory: false },
      { fieldName: "ReportedBy", fieldType: "Text", isMandatory: false },
      { fieldName: "ReportedByDate", fieldType: "Text", isMandatory: false },
      { fieldName: "ReportedByTime", fieldType: "Text", isMandatory: false },
      { fieldName: "ReportedTo", fieldType: "Text", isMandatory: false },
      { fieldName: "Action", fieldType: "Text", isMandatory: false },
      { fieldName: "ActionReason", fieldType: "Text", isMandatory: false },
      { fieldName: "ActionBy", fieldType: "Text", isMandatory: false },
      { fieldName: "ActionDate", fieldType: "Text", isMandatory: false },
      { fieldName: "ActionTime", fieldType: "Text", isMandatory: false },
      { fieldName: "ReviewedBy", fieldType: "Text", isMandatory: false },
      { fieldName: "ReviewedDate", fieldType: "Text", isMandatory: false },
      { fieldName: "ReviewedTime", fieldType: "Text", isMandatory: false },
      { fieldName: "FaultImage", fieldType: "Image", isMandatory: false },
      { fieldName: "AcknowledgedImage", fieldType: "Image", isMandatory: false },
      { fieldName: "Video", fieldType: "Text", isMandatory: false },
      { fieldName: "Location", fieldType: "Text", isMandatory: false },
      { fieldName: "Department", fieldType: "Text", isMandatory: false }
    ];
    ...
    console.log('Creating Electromagnetic Checklist with valid field types');
  ...
  return (
    <Modal show={show} onHide={handleClose} size="md">
      ...
      <Modal.Body>
        ...
        {showModuleCreation && (
          <div>
            <h6>Create New Emtelephone Checklist Module</h6>
            <p className="text-muted mb-3">
              This will create a module with predefined Emtelephone Checklist fields
            </p>
            ...
            <Form.Group className="mb-4">
              ...
              <Form.Control
                ...
                value={moduleData.name}
                onChange={(e) => setModuleData(prev => ({ ...prev, name: e.target.value }))}
                placeholder="Enter module name (e.g., Emtelephone Checklist)"
                required
              />
            </Form.Group>
            ...
            <div className="d-flex gap-2">
              <Button 
                variant="primary" 
                onClick={handleModuleCreate}
                disabled={creatingModule || !moduleData.name}
              >
                {creatingModule ? (
                  <>
                    <Spinner animation="border" size="sm" className="me-2" />
            ...
            {/* Predefined Fields Information */}
            <Alert variant="info" className="mt-3">
              <h6>Predefined Emtelephone Checklist Fields:</h6>
              <div className="row small">
                <div className="col-md-6">
                  <strong>Status Fields:</strong>
                  <ul className="mb-2">
                    <li>Status</li>
                    <li>ReportedBy</li>
                    <li>ReportedByDate</li>
                    <li>ReportedByTime</li>
                    <li>ReportedTo</li>
                  </ul>
                </div>
                <div className="col-md-6">
                  <strong>Media Fields:</strong>
                  <ul className="mb-2">
                    <li>FaultImage[]</li>
                    <li>AcknowledgedImage[]</li>
                    <li>Video[]</li>
                  </ul>
                </div>
                <div className="col-md-6">
                  <strong>Organization Fields:</strong>
                  <ul>
              ...
              <small className="text-muted">
                All these fields are predefined in the EmtelephoneChecklistSchema and will be automatically created.
              </small>
            </Alert>
          </div>
        )}
        ...
        {showExistingModules && (
          <div>
            <h6>Select Existing Module</h6>
            <p className="text-muted mb-3">Choose from previously created Emtelephone modules:</p>
                  ...
                  {(() => {
                    ...
                    return selectedModule ? (
                      <div>
                        <p><strong>Name:</strong> {selectedModule.name}</p>
                        {selectedModule.logo && (
                          <p><strong>Has Logo:</strong> Yes</p>
                        )}
                        <p><strong>Type:</strong> Emtelephone Checklist</p>
                      </div>
                    ) : null;
                  })()}
                </Card.Body>
            ...
            {existingModules.length === 0 && (
              <div className="text-center text-muted py-4 border rounded">
                <p>No existing Emtelephone modules found.</p>
                <p>Please create a new module instead.</p>
                <Button 
                  variant="outline-primary" 
                  onClick={() => {
...
const SuggestionDropdown = ({ field, suggestions, onSelect, show, onMouseDown, type = 'simple' }) => {
    ...
    >
      {suggestions.map((suggestion, index) => (
        <div
          ...
          onClick={() => onSelect(field, type === 'productName' ? suggestion : suggestion)}
          ...
          {type === 'productName' ? (
            <>
              ...
              <div className="small text-muted">
                Brand: {suggestion.brand} | Category: {suggestion.category} | Type: {suggestion.type}
              </div>
            </>
          ) : (
            suggestion
...
const ProductCreateModal = ({ 
  ...
  const handleClearAutoFill = () => {
    ...
    // Update form data using the setFormData prop
      ...
      <Form onSubmit={onSubmit}>
          ...
          {duplicateProduct && (
            <Alert variant="info" className="mb-3">
              <div className="d-flex justify-content-between align-items-center">
                <div>
                  <strong>✓ Auto-filled from existing product:</strong> {duplicateProduct.name}
                  <br />
                  <small>
                    Brand: {duplicateProduct.brand} | Category: {duplicateProduct.category} | 
            ...
            <Col md={6}>
              <Form.Group className="mb-3 position-relative">
                ...
                <Form.Control
                  ...
                  isInvalid={!!duplicateProduct}
                />
                {duplicateProduct && (
                  <Form.Text className="text-warning">
                    Similar product found: {duplicateProduct.name} - {duplicateProduct.brand} - {duplicateProduct.category}
                  </Form.Text>
                )}
                ...
                <SuggestionDropdown
                  ...
                  show={showDropdown.productNames}
        ...
        <Modal.Footer>
          ...
          <Button 
            variant="primary" 
            type="submit"
            disabled={loading || !!duplicateProduct}
          >
            {loading ? (
              <>
                <Spinner animation="border" size="sm" className="me-2" />
...
const ParentProductRequests = ({ 
  ...
  return (
    <div>
      {/* Debug Info */}
      <Alert variant="info" className="mb-3">
        <strong>Debug Info:</strong> Found {requests.length} total requests, {pendingRequests.length} pending
      </Alert>
...
// Combined Requests Tab Component - FIXED VERSION
const RequestsTab = ({ 
  ...
  const [activeRequestTab, setActiveRequestTab] = useState('authorizationRequests');
  const [actionModal, setActionModal] = useState({
    show: false,
    request: null,
    action: '',
  ...
  const handleAuthorizationAction = (requestId, action, notes = '') => {
    onAuthorization(requestId, action, notes);
  };
  ...
  const handleActionSubmit = () => {
    const { request, action, notes } = actionModal;
    handleAuthorizationAction(request._id, action, notes);
    setActionModal({ show: false, request: null, action: '', notes: '' });
  };
  ...
  return (
    <div>
      ...
      <Modal show={actionModal.show} onHide={handleCloseModal}>
        <Modal.Header closeButton>
          <Modal.Title>
            {actionModal.action === 'approve' ? 'Approve' : 'Reject'} Authorization Request
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {actionModal.request && (
            <div className="mb-3">
              ...
              <Card className="bg-light">
                <Card.Body className="py-2">
                  ...
                  <br />
                  <small>
                    Requested by: {actionModal.request.childOrganizationName}
                    <br />
                    Product: {actionModal.request.productBrand} - {actionModal.request.productCategory}
                    <br />
                    Requested: {formatDate(actionModal.request.requestedAt)}
                  </small>
                </Card.Body>
              </Card>
            </div>
          ...
          <Form.Group className="mb-3">
            ...
            <Form.Control
              ...
              placeholder={
                actionModal.action === 'approve' 
                  ? 'Add approval notes or comments...' 
                  : 'Please provide reason for rejection...'
              }
              required
            />
        ...
        <Modal.Footer>
          ...
          <Button 
            variant={actionModal.action === 'approve' ? 'success' : 'danger'}
            onClick={handleActionSubmit}
            disabled={!actionModal.notes.trim()}
          >
            Confirm {actionModal.action === 'approve' ? 'Approve' : 'Reject'}
          </Button>
        </Modal.Footer>
      ...
      >
        ...
        >
          <Card>
            <Card.Header className="bg-primary text-white">
              <h5 className="mb-0">Authorization Requests</h5>
              <small>Manage product authorization requests from your organization hierarchy</small>
            </Card.Header>
            <Card.Body>
              <Alert variant="info" className="mb-3">
                <strong>Instructions:</strong> This section handles product authorization requests from child organizations. 
                Review each request and approve or reject based on your organization's policies.
              </Alert>
              ...
              {loading ? (
                ...
                <Alert variant="success" className="text-center">
                  <h5>No Pending Authorization Requests</h5>
                  <p>All authorization requests have been processed.</p>
                </Alert>
              ) : (
                <div className="table-responsive">
                  <Table striped hover>
                        ...
                        <tr key={request._id}>
                          <td>
                            <div>
                              ...
                              <br />
                              <small className="text-muted">
                                <strong>Brand:</strong> {request.productBrand}
                                <br />
                                <strong>Category:</strong> {request.productCategory}
                                {request.productType && (
                                  <>
                                    <br />
                                    <strong>Type:</strong> {request.productType}
                                  </>
                                )}
                              </small>
                            </div>
                          ...
                          <td>
                            <div>
                              <strong>{request.childOrganizationName}</strong>
                              <br />
                              <small className="text-muted">
            ...
            <Card.Footer className="text-muted">
              <div className="d-flex justify-content-between align-items-center">
                <small>
                  Showing {authorizationRequests.length} authorization requests
                </small>
                <Badge bg="info">
                  Pending: {pendingAuthorizationCount} | 
                  Approved: {authorizationRequests.filter(r => r.status === 'Approved').length} | 
                  Rejected: {authorizationRequests.filter(r => r.status === 'Rejected').length}
                </Badge>
              </div>
            </Card.Footer>
          </Card>
